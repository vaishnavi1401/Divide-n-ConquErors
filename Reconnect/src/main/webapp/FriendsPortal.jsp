		...
		<script>
        	...
        	function pendingRequests(){
        		...
        		var data = getPendingJsonData(sessionUsername, function (obj){
        			...
        			displaytext += "<tr><th>Username</th><th>Accept</th><th>Ignore</th><th>Remove</th></tr>";
        			
        			var usrnm=obj['PendingReqList'].toString();
        			var str=String(usrnm);
		...
		function viewFriends()
		{
			console.log("in frnd");
				...
				var data = getFriendsJsonData(sessionUsername, function (obj){
	    			...
	    			var usrnm=obj['FriendsList'].toString();
	    			var str=String(usrnm);
	    			
	    			displaytext +="<tr>"+
	    			"<td> "+usrnm+ "</td>"+		
					"<td><input type=button value=remove id=remove class="+str+" onclick=removeResp(this.id)></td>"+
					"</tr>";
...
<!--href="FriendServlet?ac=nonFriends" 

async function sendResponse()
        	{
        					var ss=document.getElementById("info").value;
        					var frnd=document.querySelector('id').value;
        					var choice ="";//=document.getElementById("input[type=button]").value;
        					 console.log(frnd);
        					...
        					/* if(choice=="accept"){
        						...
        						console.log("inside accept");
        						 request = "ac=acceptFriendRequest&username="+ss+"&friend="+frnd;
        						 xhr.send(request);
        					} */
        					if(choice=="ignore"){
        						var xhr = new XMLHttpRequest();
        						...
        						console.log("inside ign");
        						 request = "ac=ignoreRequest&username="+ss+"&friend="+frnd;
        						 xhr.send(request);
        						 xhr.onreadystatechange = function() {
             	        			if (xhr.readyState == 4 && xhr.status == 200) {
        					...
        					if(choice=="remove"){
        						...
        						console.log("inside rem");
        						 request = "ac=removeFriend&username="+ss+"&friend="+frnd;
        						 xhr.send(request);
        						 xhr.onreadystatechange = function() {
             	        			if (xhr.readyState == 4 && xhr.status == 200) {
