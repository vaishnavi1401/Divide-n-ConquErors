...
public class FriendDao implements FriendDaoInterface {
	...
	{
		List<User> pendingFriendRequests=new ArrayList<User>();
		PreparedStatement pstmt = null;
		...
		try {
			...
			String sql2="select friend_one,friend_two from friend_details where (friend_one=? or friend_two=?) and status=0 ";
			pstmt=conn.prepareStatement(sql2);
